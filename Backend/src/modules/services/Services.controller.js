...
export class ServicesController {
    ...
    async createServiceRealized(req, res) {
        try {
            const payload = req.body ?? {};
    ...
    async getServicesRealized(req, res) {
        try {
    ...
    async updateServiceRealized(req, res) {
        try {
            const { id } = req.params;
            if (typeof id !== "string" || !id.trim()) {
                return res.status(400).json({
    ...
    async deleteServiceRealized(req, res) {
        try {
            const { id } = req.params;
            if (typeof id !== "string" || !id.trim()) {
                return res.status(400).json({
    ...
    async settleServiceRealized(req, res) {
        try {
            const { id } = req.params;
            if (typeof id !== "string" || !id.trim()) {
                return res.status(400).json({
