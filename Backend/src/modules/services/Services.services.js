...
import {} from "../../types/Services/Services.types.js";
...
const getDefaultServiceImage = () => {
    for (const imagePath of defaultImageCandidates) {
        try {
            ...
            console.error("Erro ao carregar imagem padrão de serviço:", imagePath, error);
        }
    }
    return null;
};
...
const sumQuantityByProduct = (items) => {
...
export class ServicesService {
    async newService(novo_servico) {
        try {
            const pool = DB.connect();
            const { nome_servico, categoria, preco, prazo, descricao, imagem, status, } = novo_servico;
            const normalizedImage = imagem && String(imagem).trim() ? imagem : defaultServiceImage;
    ...
    async updateService(id, servico) {
        try {
            const pool = DB.connect();
            if (!id) {
                return {
            ...
            const { nome_servico, categoria, preco, prazo, descricao, imagem, status, } = servico;
            const normalizedImage = imagem && String(imagem).trim() ? imagem : defaultServiceImage;
            const query = `
        UPDATE servicos
        SET
    ...
    async createServiceRealized(payload) {
        const pool = DB.connect();
        try {
      ...
      `;
        ...
        `;
            ...
            if (items.length) {
                ...
                const stockMovement = await productsService.movimentEstoque({
                    items: items.map((item) => ({
                        ...
                        description: normalizeText(payload.notes) ??
                ...
                if (!stockMovement?.success) {
                    ...
                    return {
                        success: false,
                        message: stockMovement?.message ??
                            "Erro ao movimentar estoque para o serviço realizado",
                    };
                }
            }
            if (status === "concluido") {
                const alreadyBilled = await pool.query("SELECT id FROM finance_movements WHERE service_realized_id = $1 LIMIT 1", [realized.id]);
            ...
            RETURNING *;
          `;
                    const title = normalizeText(realized.descricao) ??
                        normalizeText(realized.servico_nome) ??
                        "Serviço realizado";
                    const movementDate = normalizeText(payload.service_date) ?? new Date().toISOString();
    ...
    async updateServiceRealized(id, payload) {
        const pool = DB.connect();
        try {
            if (!id) {
                return {
            ...
            const status = normalizeText(payload.status) ?? "concluido";
            await pool.query("BEGIN");
            const previousItemsResult = await pool.query(`SELECT produto_id, quantidade
         FROM servicos_realizados_itens
         WHERE servico_realizado_id = $1`, [id]);
      ...
      `;
            ...
            const realized = serviceResult.rows[0];
            await pool.query("DELETE FROM servicos_realizados_itens WHERE servico_realizado_id = $1", [id]);
            if (items.length) {
                const insertItem = `
          INSERT INTO servicos_realizados_itens (
            servico_realizado_id,
        ...
        `;
            ...
            const newByProduct = sumQuantityByProduct(items.map((item) => ({
    ...
    async deleteServiceRealized(id) {
        try {
            const pool = DB.connect();
            if (!id) {
                return {
            ...
            await pool.query("BEGIN");
            await pool.query("DELETE FROM servicos_realizados_itens WHERE servico_realizado_id = $1", [id]);
            await pool.query("DELETE FROM finance_movements WHERE service_realized_id = $1", [id]);
            const result = await pool.query("DELETE FROM servicos_realizados WHERE id = $1 RETURNING *", [id]);
            await pool.query("COMMIT");
            return {
                success: true,
    ...
    async getServicesRealized() {
        try {
            const pool = DB.connect();
            const servicesResult = await pool.query(`SELECT * FROM servicos_realizados ORDER BY criado_em DESC`);
            ...
            const itemsResult = await pool.query(`SELECT * FROM servicos_realizados_itens WHERE servico_realizado_id = ANY($1::uuid[])`, [ids]);
    ...
    async settleServiceRealized(id, payload) {
        const pool = DB.connect();
        try {
            if (!id) {
                return {
            ...
            await pool.query("BEGIN");
            const serviceResult = await pool.query("SELECT * FROM servicos_realizados WHERE id = $1 FOR UPDATE", [id]);
            ...
            const alreadyBilled = await pool.query("SELECT id FROM finance_movements WHERE service_realized_id = $1 LIMIT 1", [id]);
            const totalValue = normalizeDbNumber(service.valor_total) ??
          ...
          RETURNING *;
        `;
                const title = normalizeText(service.descricao) ??
            ...
            const updatedServiceResult = await pool.query(`UPDATE servicos_realizados
         SET status = $1, atualizado_em = NOW()
         WHERE id = $2
         RETURNING *`, ["concluido", id]);
            await pool.query("COMMIT");
            return {
                success: true,
