...
export class FinanceService {
    ...
    async updateMovement(id, payload) {
        try {
            ...
            const existingResult = await pool.query("SELECT * FROM finance_movements WHERE id = $1", [id]);
            ...
            const movementDate = normalizeText(payload.date) ?? existing.movement_date ?? new Date().toISOString();
            const update = `
    ...
    async deleteMovement(id) {
        try {
            ...
            const result = await pool.query("DELETE FROM finance_movements WHERE id = $1 RETURNING *", [id]);
            return {
                success: true,
...
//# sourceMappingURL=Finance.services.js.map