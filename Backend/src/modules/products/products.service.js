...
import {} from "../../types/Products/Products.types.js";
...
export class ProductsService {
    async newProduct(produto) {
        try {
            const pool = DB.connect();
            const { codigo, nome, categoria, sku, preco_venda, custo, estoque, unidade, descricao, imagem, ativo, } = produto;
            const requiredNome = nome?.toString().trim();
            const requiredPrecoVenda = normalizeNumber(preco_venda);
            const normalizedImage = imagem && String(imagem).trim() ? imagem : defaultProductImage;
            if (!requiredNome) {
                return {
                    success: false,
    ...
    async deleteProduct(id) {
      ...
      `;
            ...
            if (error?.code === "23503") {
                return {
                    success: false,
                    message: "Não é possível excluir este produto porque ele possui registros relacionados",
                    code: error?.code,
                    constraint: error?.constraint,
                };
            }
    ...
    async movimentEstoque(params) {
        ...
        const descriptionsByProduct = items.reduce((acc, item) => {
            const productId = item.product_id ? String(item.product_id) : null;
        ...
        try {
            ...
            for (const productId of productIds) {
                ...
                if (quantity <= 0)
                    continue;
                const productResult = await runner.query("SELECT id, nome, estoque FROM produtos WHERE id = $1 FOR UPDATE", [productId]);
                ...
                const currentStock = movementType === "entrada"
                    ? previousStock + quantity
                    : previousStock - quantity;
                await runner.query("UPDATE produtos SET estoque = $1 WHERE id = $2", [currentStock, productId]);
                await runner.query(`INSERT INTO estoque_movimentacoes (
            produto_id,
            tipo,
            quantidade,
            saldo_anterior,
    ...
    async moveStockByProduct(payload) {
        try {
