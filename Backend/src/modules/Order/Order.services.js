...
import {} from "../../types/Order/Order.types.js";
...
const generateOrderPdf = async (order, items) => {
    ...
    items.forEach((item) => {
        doc
            .fontSize(10)
            .text(`${item.produto_nome ?? "Produto"} - Qtd: ${item.quantidade ?? 0} - R$ ${item.preco_unitario ?? 0} - Total: R$ ${item.total_item ?? 0}`);
    });
    doc.end();
...
export class OrderService {
    async createOrder(payload) {
        ...
        try {
            ...
            const shouldUseServiceFallbackPrice = (payload.service_value === null ||
            ...
            if ((!serviceDescription || shouldUseServiceFallbackPrice) && payload.service_id) {
                try {
                    const serviceResult = await pool.query("SELECT nome_servico, preco FROM servicos WHERE id = $1", [payload.service_id]);
            ...
            const estimatedValue = payload.estimated_value !== null &&
    ...
    async getOrders() {
        try {
            const pool = DB.connect();
            const ordersResult = await pool.query("SELECT * FROM orcamentos ORDER BY criado_em DESC");
            ...
            const itemsResult = await pool.query("SELECT * FROM orcamentos_itens WHERE orcamento_id = ANY($1::uuid[])", [ids]);
    ...
    async updateOrder(id, payload) {
        ...
        try {
            ...
            if ((!updateServiceDescription || shouldUseServiceFallbackPrice) && payload.service_id) {
                try {
                    const serviceResult = await pool.query("SELECT nome_servico, preco FROM servicos WHERE id = $1", [payload.service_id]);
    ...
    async deleteOrder(id) {
        try {
            ...
            const result = await pool.query("DELETE FROM orcamentos WHERE id = $1 RETURNING *", [id]);
            return {
                success: true,
    ...
    async convertToServiceRealized(id) {
        const pool = DB.connect();
        try {
            if (!id) {
                return {
            ...
            await pool.query("BEGIN");
            const orderResult = await pool.query("SELECT * FROM orcamentos WHERE id = $1", [id]);
            const order = orderResult.rows[0];
            if (!order) {
                await pool.query("ROLLBACK");
                return {
            ...
            const itemsResult = await pool.query("SELECT * FROM orcamentos_itens WHERE orcamento_id = $1", [id]);
            const items = itemsResult.rows ?? [];
            const insertService = `
      ...
      `;
        ...
        `;
            ...
            if (items.length) {
                ...
                const stockMovement = await productsService.movimentEstoque({
                    items: items.map((item) => ({
                        ...
                        description: normalizeText(order.observacoes) ??
            ...
            await pool.query("UPDATE orcamentos SET status = $1, atualizado_em = NOW() WHERE id = $2", ["convertido", id]);
            await pool.query("COMMIT");
            return {
                success: true,
    ...
    async exportOrderToWhatsapp(id) {
        try {
            const pool = DB.connect();
            ...
            if (!token || !phoneId) {
                return {
                    success: false,
                    message: "Configuração do WhatsApp ausente. Defina WHATSAPP_TOKEN e WHATSAPP_PHONE_NUMBER_ID.",
                };
            }
            const orderResult = await pool.query("SELECT * FROM orcamentos WHERE id = $1", [id]);
            const order = orderResult.rows[0];
            ...
            form.append("file", new Blob([new Uint8Array(pdfBuffer)], { type: "application/pdf" }), `orcamento-${order.id}.pdf`);
            form.append("type", "application/pdf");
            const mediaResponse = await fetch(`https://graph.facebook.com/v19.0/${phoneId}/media`, {
                method: "POST",
                headers: {
            ...
            const sendResponse = await fetch(`https://graph.facebook.com/v19.0/${phoneId}/messages`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${token}`,
